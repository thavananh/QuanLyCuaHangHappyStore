...
namespace GUI
{
    public partial class frmEquipmentInfo : Form
    {
        ...
        public frmEquipmentInfo(string maTB)
        {
            InitializeComponent();
            this.maTB = maTB;
        ...
        private bool checkupdateanh = false;
        ...
        {
            tb = tbll.xemThongTinTB(maTB);
            tbEquipmentId.Text = tb.Mathietbi;
            tbEquipmentId.Enabled = false;
            cbEquipmentCondition.SelectedItem = tb.Tinhtrang;
            tbEquipmentName.Text = tb.Tenthietbi;
            using (MemoryStream ms = new MemoryStream(tb.Anh))
            {
                ptbTB.Image = Image.FromStream(ms);
            }
            foreach (var item in cbEquipmentType.Items)
            {

                // Lấy mã thiết bị
                LOAITHIETBI temp = item as LOAITHIETBI;
                if (temp.MaLoaiThietBi == tb.Loaithietbi)
                {
                    cbEquipmentType.SelectedItem = item;
                    break;
                }
            }
        }
        ...
        private void fThongtinTB_Load(object sender, EventArgs e)
        {
        ...
        private void btnChinhSua_Click(object sender, EventArgs e)
        {
            ...
            string checkupdate = tbll.checkcapnhat(tb);
            switch(checkupdate)
            {
                case "success":
                    ...
                    return;
                case "fail":
                    MessageBox.Show("update that bai", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;
            }
        }
        ...
        {
            ptbTB.Image = (Bitmap)e.Frame.Clone();
        }
        ...
        private void SaveQRCodeToFile()
        {
            ...
            saveFileDialog.FileName = "QRCode_" + tb.Tenthietbi + ".png";
